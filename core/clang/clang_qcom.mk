ifneq ($(LOCAL_MODULE),$(filter $(LOCAL_MODULE),$(CLANG_QCOM_DONT_USE_MODULES)))
  ...
  arm_objects_cflags_CLANG_QCOM := $(call convert-to-clang-qcom-flags,$($(LOCAL_2ND_ARCH_VAR_PREFIX)$(my_prefix)$(arm_objects_mode)_CFLAGS)) $(CLANG_QCOM_CONFIG_KRAIT_FLAGS)
  ...
  ifeq ($(LOCAL_MODULE),libcompiler_rt)
    my_asflags_CLANG_QCOM  += -mcpu=cortex-a15 -v
    #my_target_global_cppflags += -mcpu=cortex-a15 -v
  endif
  ...
  ifeq ($(LOCAL_MODULE),$(filter $(LOCAL_MODULE),$(CLANG_QCOM_ALIGN-OS_MODULES)))
    #my_target_global_cflags := $(filter-out -falign-os,$(my_target_global_cflags))
    #my_target_global_cppflags := $(filter-out -falign-os,$(my_target_global_cppflags))
    #my_target_global_ldflags := $(filter-out -falign-os,$(my_target_global_ldflags))
    #my_cflags_CLANG_QCOM := $(filter-out -falign-os,$(my_cflags_CLANG_QCOM))
    #my_cppflags_CLANG_QCOM := $(filter-out -falign-os,$(my_cppflags_CLANG_QCOM))
    #my_asflags_CLANG_QCOM := $(filter-out -falign-os,$(my_asflags_CLANG_QCOM))
    #my_ldflags_CLANG_QCOM := $(filter-out -falign-os,$(my_ldflags_CLANG_QCOM))
    #LOCAL_CONLYFLAGS := $(filter-out -falign-os,$(LOCAL_CONLYFLAGS))
    #arm_objects_cflags_CLANG_QCOM := $(filter-out -falign-os,$(arm_objects_cflags_CLANG_QCOM))
